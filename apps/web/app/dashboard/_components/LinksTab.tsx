...
import { 
  Plus, Loader2, Link2, Layers, Share2, CreditCard, ChevronDown, GripVertical, MousePointer2, ExternalLink, Check, Copy, Edit, Trash2, X, AlertCircle, Save, Zap, Code2, Briefcase, FileText, Music, ShoppingBag, MessageSquare, Globe
} from 'lucide-react'
import {
  Sheet,
  SheetContent,
...
const PLATFORM_DISPLAY_CATEGORY_UI: Record<string, string> = {
  instagram: 'Social', facebook: 'Social', linkedin: 'Social', youtube: 'Social',
  x: 'Social', twitter: 'Social', threads: 'Social', tiktok: 'Social',
  snapchat: 'Social', pinterest: 'Social', tumblr: 'Social', bluesky: 'Social', mastodon: 'Social',
  whatsapp: 'Messaging', telegram: 'Messaging', discord: 'Messaging', signal: 'Messaging',
...
const getCategoryIcon = (category: string) => {
  switch (category) {
    ...
    case 'Messaging': return <MessageSquare className="w-3.5 h-3.5 shrink-0" />
...
const getUiCategory = (dbCat: string, platformName: string): string => {
  ...
  return PLATFORM_DISPLAY_CATEGORY_UI[normalized] || 'Social'
}
...
export function LinksTab() {
  ...
  // Duplicating from others dialog state
  ...
  // Autodetect platform details from URL
  useEffect(() => {
    if (linkModal.open && linkModal.mode === 'add' && linkForm.url.trim()) {
  ...
  }, [linkForm.url, linkModal.mode, linkModal.open]);
        ...
        if (isChecked) {
          if (!existingJunction) {
            ...
            if (profId === activeProfile.id) {
              setProfileLinks((prev: any[]) => prev.map(l => l.link_id === linkId ? {
                ...l,
                category: dbCategory,
                platform: linkForm.platform.trim(),
                label: linkForm.label.trim() || null,
          ...
          if (existingJunction) {
            ...
            if (profId === activeProfile.id) {
              setProfileLinks((prev: any[]) => prev.filter(l => l.id !== existingJunction.id))
            }
          }
        }
      })
  ...
  const toggleLinkActive = async (link: any) => {
    ...
    setProfileLinks((prev: any[]) => prev.map(l => l.id === link.id ? { ...l, is_active: newVal } : l))
    await supabase.from('profile_links').update({ is_active: newVal }).eq('id', link.id)
  }

  const deleteLink = async (id: string) => {
    setProfileLinks((prev: any[]) => prev.filter(l => l.id !== id))
  ...
  const handleCategoryReorder = (categoryKey: string, newCategoryLinks: any[]) => {
  ...
  const confirmDuplicateLinkFromOthers = async () => {
    ...
    try {
      ...
      setMessageType('success')
      setMessage(`Link duplicated successfully!`)
    } catch (err: any) {
      setMessageType('error')
      setMessage(err.message || 'Failed to duplicate link.')
    } finally {
  ...
  if (isAllCards) return (
    <div className="flex flex-col items-center justify-center py-24 text-center space-y-5">
      <div className="w-16 h-16 rounded-2xl bg-[#3f5ce6]/10 border border-[#3f5ce6]/20 flex items-center justify-center">
        <Link2 size={28} className="text-[#3f5ce6]" />
      </div>
      <div>
        <h3 className="text-sm font-bold text-foreground">Select a Card Workspace</h3>
        <p className="text-xs text-muted-foreground mt-1 max-w-xs leading-relaxed">
          Manage Links is per-card. Please select a specific card from the workspace dropdown in the header to view and edit its links.
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-6 animate-fadeIn text-left w-full">
      ...
      {profileLinksLoading ? (
        ...
        /* Empty state */
        <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 border border-dashed border-border rounded-2xl bg-card">
        ...
        <div className="space-y-3">
          {([
            { key: 'Social', label: 'Social & Messaging', icon: <Share2 size={13} />, bg: 'bg-blue-500/15', tc: 'text-blue-400' },
            ...
            return (
              <div key={catKey} className="bg-card border border-border/80 rounded-xl overflow-hidden shadow-sm">
                <button
                  type="button"
                  onClick={() => setCollapsedCategories(prev => ({ ...prev, [catKey]: !isCatCollapsed }))}
                  className="w-full px-5 py-3 flex items-center justify-between bg-muted/40 dark:bg-zinc-900/10 select-none hover:bg-muted dark:hover:bg-zinc-800/20 transition-colors text-left"
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="inline-flex items-center rounded-md bg-muted dark:bg-zinc-850 text-muted-foreground text-[10px] uppercase font-bold px-2 py-0.5">
                      {catLinks.length}
                    </span>
                  </div>
                    ...
                    <Reorder.Group axis="y" values={catLinks} onReorder={(newOrder) => handleCategoryReorder(catKey, newOrder)} className="space-y-2">
                      {catLinks.map((link) => (
                        <Reorder.Item
                          key={link.id}
                          value={link}
                          className={`group flex items-center bg-card border border-border dark:border-zinc-850 rounded-lg px-3 shadow-sm cursor-grab active:cursor-grabbing hover:border-zinc-350 dark:hover:border-zinc-800 transition-colors min-h-[64px] ${!link.is_active ? 'opacity-60' : ''}`}
                        >
                          {/* Grip handle */}
                          ...
                          {/* Label */}
                          <div className="flex flex-col min-w-0 flex-1 py-3 select-none justify-center text-left">
                          ...
                          <div className="flex items-center gap-3 shrink-0 ml-4" onClick={e => e.stopPropagation()}>
                            ...
                            {linkPendingDelete === link.id ? (
                              <div className="flex items-center gap-1 animate-fadeIn">
                                ...
                                <button onClick={() => setLinkPendingDelete(null)} className="h-8 w-8 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-650 dark:text-zinc-350 hover:bg-zinc-200 dark:hover:bg-zinc-700 flex items-center justify-center transition-all cursor-pointer" title="Cancel">
                                  <X className="w-4 h-4" />
                                </button>
                              </div>
                            ) : (
          ...
          <SheetFooter className="p-6 border-t border-border dark:border-zinc-800 bg-muted/40 dark:bg-zinc-950/40 shrink-0 mt-auto flex-row sm:space-x-2 justify-end">
            <div className="flex gap-3 w-full">
              <button
                ...
                className="flex-grow py-2.5 rounded-xl border border-border dark:border-zinc-800 text-muted-foreground dark:text-zinc-450 hover:text-foreground dark:hover:text-white hover:bg-muted/50 dark:hover:bg-zinc-800/40 text-xs font-semibold cursor-pointer text-center transition-all active:scale-98"
              >
                Cancel
              </button>
              <button
